...
const CATEGORY_ORDER = ["Anillos", "Aretes", "Aros", "Collares", "Pulseras", "Carteras", "Otros"];
...
export function ProductGrid({ products, currencySymbol }: Props) {
  ...
  const categoryLabels = React.useMemo(() => {
    const present = new Set(products.map((p) => (p.category ?? "").trim()).filter(Boolean));
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {categoryLabels.map((category) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm shadow-soft ${
                activeCategory === category ? "border-ink bg-ink text-parchment" : "border-black/10 bg-white"
              }`}
            >
              {category}
            </button>
